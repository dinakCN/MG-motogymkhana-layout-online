...
| В таблице «сход» или зачёркнутое время | Так и должно быть. Заваленную попытку сайт зачёркивает — в кадре она тоже зачёркнута; незавершённую записывает как `59:59.99` — в кадре это слово «сход». В зачёт не идёт ни та, ни другая |
...
```bash
npm test          # 152 теста: парсер, состояние, опросчик, арифметика и порядок, моменты дня
npm run test:watch
```
