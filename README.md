...
**Система показывает протокол, а не ведёт его.** В графике нет ни одного
поля для ввода времени: результаты приходят со страницы этапа на
gymkhana-cup.ru, где секретарь соревнований ведёт протокол по ходу дня.
Сервер опрашивает её раз в несколько секунд и рассылает изменения в кадр.

Отсюда две роли, работающие параллельно и независимо:
...
Оператор графики за весь день не вводит ни одной цифры. Он вызывает
райдера по номеру и переключает сцены — места, лучшее время и рейтинги
пересчитываются сами, заходить на сайт ему незачем. Совмещать роли можно,
и один человек справится на небольшом старте, но в эфире это две разные
работы: пока секретарь ищет строку в протоколе, в кадре некому переключить
камеру на следующего участника.

**Что требуется от организации соревнований:** результат должен попадать
на сайт **сразу после заезда**, а не пачками в конце серии. Иначе весь
день в кадре будет стоять таблица получасовой давности — графика покажет
ровно то, что успели внести. Это единственное требование системы к
организаторам, и оно же — главное условие того, что трансляция выглядит
живой.

Единственное, что приходит мимо сайта, — живой отсчёт во время заезда:
его даёт секундомер на трассе, подключённый напрямую (см. «Как подключить
таймер»). Прибора может и не быть — тогда в зоне времени стоит результат
первой попытки.
