...
//
// Зачем скрипт, если то же делается в веб-интерфейсе: он пишет сеть и режим
// одним запросом, а потом сам находит прибор в новой сети и печатает готовую
// строку `timer: '…'` для event.config.js. Руками пришлось бы искать адрес,
// который выдал роутер. Если в прошивке нет поля `wifi.wifiid`, добавленная
// в интерфейсе сеть сохранится, но не включится — тогда скрипт единственный
// путь.
