...
/**
 * @deprecated Колонка в кадре заезда считается по группе награждения
 * (`topOfGroup` в awardGroups.js) с 298d58e, и потребителей здесь не
 * осталось. Функция оставлена на случай возврата к классовому топу:
 * правило «спортсмен не выпадает из собственного топа» стоило отдельного
 * разбора, и восстанавливать его по памяти дороже, чем держать готовым.
 * Тесты в format.test.js её по-прежнему проверяют — иначе ко дню возврата
 * она приедет молча сломанной.
 */
