...
export function loadConfig(env = process.env, event = eventConfig) {
  ...
  return {
    ...
    // Жёсткое разделение — свойство положения о соревновании, а не решение
    // оператора в эфире: правится тем же файлом, что и состав групп.
    // Умолчание мягкое, потому что Круизер и SB определяет мотоцикл,
    // а не мастерство, и они совмещаются с классовым зачётом.
    strictGroups: event.strictGroups === true,
  }
}
