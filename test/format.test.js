...
// ГЛАВНОЕ ПРАВИЛО ЗАЧЁТА. В итоговое время идёт только пройденная и
// засчитанная попытка: её время из протокола плюс штраф, если он есть.
// Всё остальное — незачёт, сход, пустая ячейка — не попадает в результат
// ни при каких условиях. Этот блок ломается, если появится обход.
describe('в зачёт идёт только засчитанная попытка', () => {
  const случаи = [
    ['засчитанная попытка', { n: 1, time: '01:23.72', penalty: 0 }, 83.72],
    ['засчитанная со штрафом', { n: 1, time: '01:23.72', penalty: 4 }, 87.72],
    ['штраф отсутствует — считаем как ноль', { n: 1, time: '01:23.72', penalty: null }, 83.72],
    ['незачёт (зачёркнута)', { n: 1, time: '01:23.72', penalty: 0, scratched: true }, null],
    ['незачёт со штрафом', { n: 1, time: '01:23.72', penalty: 4, scratched: true }, null],
    ['сход', { n: 1, time: '59:59.99', penalty: 0 }, null],
    ['сход, помеченный зачёркиванием', { n: 1, time: '59:59.99', penalty: 0, scratched: true }, null],
    ['пустая ячейка', { n: 1, time: null, penalty: null }, null],
    ['пустое время при штрафе', { n: 1, time: null, penalty: 5 }, null],
    ['время в чужом формате', { n: 1, time: '83.72', penalty: 0 }, null],
  ]

  it.each(случаи)('%s', (_имя, attempt, ожидание) => {
    const total = attemptTotal(attempt)
    if (ожидание === null) expect(total).toBeNull()
    else expect(total).toBeCloseTo(ожидание, 4)
  })

  it('лучшее время участника собирается только из засчитанных попыток', () => {
    const p = rider({ attempts: случаи.map(([, attempt], i) => ({ ...attempt, n: i + 1 })) })

    // среди случаев есть 01:23.72+4 = 87.72 и 01:23.72 = 83.72
    expect(bestSeconds(p)).toBeCloseTo(83.72, 2)
  })
})
...
describe('formatClock', () => {
  ...
  it('десятые — то, что успевает показать кадр на 25 fps', () => {
    expect(formatClock(83.7, 1)).toBe('01:23.7')
    expect(formatClock(83.04, 1)).toBe('01:23.0')
    expect(formatClock(0, 1)).toBe('00:00.0')
  })

  // Округление вверх дало бы 01:24.0 на 83.99 — цифра больше настоящей,
  // а следом финиш 01:23.99 выглядел бы шагом назад.
  it('доли отбрасываются и с десятыми', () => {
    expect(formatClock(83.99, 1)).toBe('01:23.9')
    expect(formatClock(59.99, 1)).toBe('00:59.9')
  })

  it('переход через минуту не теряет ноль', () => {
    expect(formatClock(59.95, 1)).toBe('00:59.9')
    expect(formatClock(60, 1)).toBe('01:00.0')
  })
})
